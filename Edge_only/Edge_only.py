...
# 将所有任务卸载至UAV ( Offload-only )：
# 在每个时隙内，UAV在区域中心固定位置向UE提供计算服务。
# UE将其所有计算任务卸载到无人机上的MEC服务器。
...
class UAVEnv(object):
    ...
    #task_list = np.random.randint(2097153, 2621440, M)  # 随机计算任务2~2.5Mbits
    # ue位置转移概率
