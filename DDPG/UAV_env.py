...
class UAVEnv(object):
    ...
    # P[W] = 10^(0.1*P[dbm]-3)
    # Note：P_los = 10^(0.1*(-100)-3) = 10^(-13)
    #       P_nlos = 10^(0.1*(-80)-3) = 10^(-11)
    ...
    def step(self, action):
        ...
        reset_dist = False
        # 将取值区间位-1~1的action -> 0~1的action。避免原来action_bound为[0,1]时训练actor网络tanh函数一直取边界0
        action = (action + 1) / 2  
        # 控制变量映射到各个变量的取值区间
        if action[0] == 1:
            ue_id = self.M - 1
        ...
        return self._get_obs(), reward, is_terminal, step_redo, offloading_ratio_change, reset_dist, ue_id
    ...
    def com_delay(self, loc_ue, loc_uav, offloading_ratio, task_size, block_flag):
        ...
        # 信噪比：self.p_uplink * g_uav_ue / p_noise
