...
class UAVEnv(object):
    ...
    sum_task_size = 100 * 1048576  # 总计算任务60 Mbits
    loc_uav = [50, 50]
    bandwidth_nums = 1
    ...
    def step(self, action):  # 0: 选择服务的ue编号 ; 1: 方向theta; 2: 距离d; 3: offloading ratio
        ...
        offloading_ratio = (action % 11) * 0.1  # ue卸载率
        task_size = self.task_list[ue_id]
        block_flag = self.block_flag_list[ue_id]

        # 飞行能耗
        e_fly = (
                            dis_fly / self.t_fly) ** 2 * self.m_uav * self.t_fly * 0.5  # ref: Mobile Edge Computing via a UAV-Mounted Cloudlet: Optimization of Bit Allocation and Path Planning
        ...
        if self.sum_task_size == 0:  # 计算任务全部完成
            ...
            reward = 0
            step_redo = True
        elif self.e_battery_uav < e_fly or self.e_battery_uav - e_fly < e_server:  # uav电量不能支持飞行或者计算
            ...
            reset_offload_ratio = True
            return self._get_obs(), reward, is_terminal, step_redo, reset_offload_ratio
